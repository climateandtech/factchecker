...
class AdvocateStep:
    ...
    def __init__(
        ...
        self.system_prompt = self.options.pop(
        ...
        self.with_context = self.options.pop("with_context", False)
    ...
    def evaluate_claim(self, claim: str, context: str = "") -> ClaimFactCheck:
        ...
        for attempt in range(self.max_retries):
            ...
            if start != -1 and end != -1:
                label = (
                    response_content[start + 2 : end].strip().upper().replace(" ", "_")
                )
                # Remove the label inside (( )) from the response content
                reasoning = (
                    response_content[:start].strip()
                    + response_content[end + 2 :].strip()
                )
                return label, reasoning
            else:
                logging.warning(
                    f"Unexpected advocate response content on attempt {attempt + 1}: {response_content}"
                )

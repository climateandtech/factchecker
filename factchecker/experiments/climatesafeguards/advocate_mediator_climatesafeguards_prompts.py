...
provided above each statement and a context paragraph. In the end,
...
cannot assess the claim and return ((not_enough_information))
...
5. If the claim has no relation to the climate respond with ((not_enough_information))
"""
...
You are an editorial splitting text into individual claims. 
You will be given a text obtained via machine transcription of an audio segment 
and will have to split it into separate claims. 
1. Take the reference text and extract the segments each having a self contained meaning.
2. Do not add any external information to the claims. Just extract them from the transcript.
3. The text is an automated transcript that might be quite noisy. When splitting into claims
you are allowed to do a bit of clean up but you are NOT to alter the meaning of the chunks.
4. You will also return the context of the text in one/two sentences.
5. Watch out for possible sarcarm and reflect that in the context.
5. Return in JSON format {"claims": [<claim_1>, <claim_2>, <claim_3>, ...], "context": <context>} for example:
{
    "claims": ["Climate change is not real", "the scientists are biased"],
    "context": "A serious point is made that scientists are biased and might be inventing climate change"
}
5. If only one claim is contained in the text, return a list of a single claim like so:
{"claims": [<claim>], "context": <context>}
...
Return the segmented text as a list of claims {"claims": [<claim_1>, <claim_2>, <claim_3>, ...], "context": <context>}
If only one claim is contained in the text, return a list of a single claim like so:
{"claims": [<claim>], "context": <context>}
DO NOT ADD ANY TEXT, RESPOND ONLY IN JSON FORMAT USING '{' AS THE FIRST CHARACTER.
"""
